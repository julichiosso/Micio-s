...
export function TurnosAdminView({ estadoInicial, turnosIniciales }: Props) {
        ...
        <div className={`rounded-xl border p-3 transition-all ${
          ...
          <div className="flex items-center justify-between mb-1.5">
            <div className="flex items-center gap-2">
              <span className="text-[14.5px] font-black text-white tracking-tight">{t.horaInicio} a {t.horaFin}</span>
              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${badgeStyle}`}>{estadoTexto}</span>
            </div>
            {!t.pasado && (
              <button type="button" onClick={() => handleToggleBloqueo(t.id, t.bloqueado)}
                className={`text-[11px] font-bold px-2 py-0.5 rounded border transition-colors cursor-pointer active:opacity-70 ${
                  t.bloqueado ? "bg-white/[0.1] border-white/20 text-white" : "bg-white/[0.05] border-white/[0.08] text-white/50 hover:text-red-400"
                }`}>
          ...
          {!t.pasado && (
            <div className="flex items-center justify-between pt-1.5 mt-1.5 border-t border-white/[0.05] text-[11.5px]">
              <span className="text-white/50 font-medium">Límite de pedidos:</span>
              <div className="flex items-center gap-1.5">
                <button type="button" onClick={() => handleCambiarCapacidad(t.id, -1)} disabled={t.capacidad <= 1 || isPending}
                  className="w-7 h-7 rounded-lg bg-white/[0.1] hover:bg-white/[0.18] active:scale-95 text-white font-bold flex items-center justify-center transition-all disabled:opacity-25 cursor-pointer text-[14px]">-</button>
                <span className="w-5 text-center font-black text-white text-[14px]">{t.capacidad}</span>
                <button type="button" onClick={() => handleCambiarCapacidad(t.id, 1)} disabled={isPending}
                  className="w-7 h-7 rounded-lg bg-white/[0.1] hover:bg-white/[0.18] active:scale-95 text-white font-bold flex items-center justify-center transition-all cursor-pointer text-[14px]">+</button>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <>
      {/* ═══════════════════════════════════════════════
          VISTA MOBILE — 100% OSCURA Y MINIMIZABLE
      ═══════════════════════════════════════════════ */}
      <div className="md:hidden px-4 pt-6 pb-24 flex flex-col gap-6 max-w-lg mx-auto">
        ...
        <div className="bg-[#1a1814] border border-white/[0.1] rounded-2xl p-5 shadow-sm">
          ...
          <button type="button" onClick={handleToggleAbierto} disabled={isPending}
            ...
            <span className={`w-2.5 h-2.5 rounded-full ${abierto ? "bg-[#141210] animate-pulse" : "bg-white"}`} />
          ...
          <div className="pt-4 mt-4 border-t border-white/[0.08] flex flex-col gap-2.5">
            <div className="flex items-center justify-between">
              <span className="font-bold text-white/80 text-[12px]">Horario de atención</span>
              {horarioGuardadoOk && (
            ...
            <div className="flex gap-2 items-center">
              <div className="flex-1">
                <label className="text-[10px] text-white/40 uppercase tracking-wider font-bold mb-1 block">Abre</label>
                <input type="time" value={horaApertura} onChange={(e) => setHoraApertura(e.target.value)}
                  className="w-full bg-white/[0.06] border border-white/[0.12] rounded-xl px-3 py-2.5 text-[14px] text-white outline-none focus:border-[#c6f135]/60 transition-colors" />
              </div>
              <span className="text-white/30 text-[12px] font-bold mt-4">a</span>
              <div className="flex-1">
                <label className="text-[10px] text-white/40 uppercase tracking-wider font-bold mb-1 block">Cierra</label>
              ...
              <button type="button" onClick={handleGuardarHorario} disabled={guardandoHorario || isPending}
                className="bg-[#c6f135] text-[#141210] rounded-xl px-3.5 py-2.5 text-[12px] font-bold hover:opacity-90 active:scale-95 transition-all mt-5 shrink-0 cursor-pointer disabled:opacity-50">
                {guardandoHorario ? "..." : "Guardar"}
              </button>
            </div>
            <p className="text-[10.5px] text-white/30 leading-snug">
              El cambio de horario aplica a partir de mañana para los pedidos. El indicador de abierto/cerrado cambia inmediatamente.
            </p>
          </div>
          ...
          <div className="pt-4 mt-4 border-t border-white/[0.08] flex flex-col gap-2.5">
            <div className="flex items-center justify-between text-[12px]">
              <span className="font-bold text-white/80">Aviso o demora para clientes</span>
              {mensajeGuardadoOk && (
                <span className="text-[#c6f135] text-[11.5px] font-bold">✓ Guardado</span>
              )}
            </div>
            <div className="flex gap-2">
              ...
              {mensaje && (
                <button type="button" onClick={() => { setMensaje(""); handleGuardarMensaje(""); }}
                  className="bg-white/[0.08] text-white/60 hover:text-white rounded-xl px-3 py-2.5 text-[12px] font-bold transition-colors cursor-pointer" title="Borrar aviso">✕</button>
              )}
            </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="px-1">
            <h3 className="text-[18px] font-black text-white leading-tight">Pedidos por Horario (Hoy)</h3>
            ...
            {gruposPorHora.map((grupo) => {
              ...
              return (
                <div key={grupo.hora} className="border border-white/[0.1] rounded-2xl bg-[#1a1814] overflow-hidden shadow-sm">
                  <button type="button" onClick={() => toggleBloque(grupo.hora)}
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[11px] text-white/40 font-medium">{estaAbierto ? "Minimizar" : "Ver horarios"}</span>
                      <div className={`w-6 h-6 rounded-full bg-white/[0.06] flex items-center justify-center transition-transform duration-200 ${
