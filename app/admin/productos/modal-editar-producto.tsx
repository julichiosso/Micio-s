...
export function ModalEditarProducto({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      {/* Modal — full-flex column, max height 92vh, sin overflow-hidden para que el footer quede siempre visible */}
      <div className="relative bg-[#1a1814] md:bg-white text-white md:text-gray-900 border border-white/[0.12] md:border-gray-200 rounded-2xl md:rounded-3xl w-full max-w-lg flex flex-col shadow-2xl z-10 animate-in fade-in zoom-in-95 duration-150"
        style={{ maxHeight: "92dvh" }}
      >
        {/* ── Cabecera fija ── */}
        <div className="px-5 py-3.5 border-b border-white/[0.08] md:border-gray-100 flex items-center justify-between bg-[#141210] md:bg-gray-50/90 shrink-0 rounded-t-2xl md:rounded-t-3xl">
        ...
        <div className="overflow-y-auto flex-1 p-4 sm:p-5">
          <form id="form-editar" onSubmit={handleGuardarTodo} className="flex flex-col gap-4">
            {/* Fila 1: Foto + Nombre */}
            <div className="flex items-start gap-3.5">
              ...
              <div className="shrink-0 flex flex-col items-center gap-1.5">
                <div
                  ...
                  className="w-16 h-16 rounded-xl bg-white/[0.05] md:bg-gray-100 border border-white/[0.1] md:border-gray-200 relative overflow-hidden flex items-center justify-center cursor-pointer hover:opacity-85 transition-opacity group"
                  title="Cambiar foto"
                >
                  {previewUrl ? (
                    <Image
                ...
                {/* Indicador de foto nueva seleccionada — siempre visible */}
                {archivoSeleccionado ? (
                  <span className="text-[9.5px] text-[#c6f135] font-bold text-center leading-tight">
                    Nueva foto<br />lista ✓
                  </span>
                ) : (
                  <span className="text-[9px] text-white/30 md:text-gray-400 text-center">
                    Tocar para<br />cambiar
                  </span>
                )}
                <input
                  ref={fileInputRef}
        ...
        {/* ── Footer fijo con botones — SIEMPRE VISIBLE, fuera del scroll ── */}
        <div className="px-4 sm:px-5 py-3.5 border-t border-white/[0.06] md:border-gray-100 flex items-center justify-end gap-2.5 shrink-0 bg-[#141210]/60 md:bg-gray-50/80 rounded-b-2xl md:rounded-b-3xl">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2.5 rounded-xl text-white/60 md:text-gray-600 hover:text-white md:hover:text-gray-900 text-[13px] font-semibold transition-colors cursor-pointer"
          >
            Cancelar
          </button>
          <button
            ...
            form="form-editar"
            disabled={guardando}
            className="px-5 py-2.5 rounded-xl bg-[#c6f135] hover:bg-[#d6ff47] text-[#141210] font-bold text-[13.5px] transition-all disabled:opacity-50 cursor-pointer active:scale-95 shadow-sm"
          >
